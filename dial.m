function dial(teclas,fs)
%% Dial generates a vector containing the signal of the specified tones, 
...
% Function Call - Ex: dial('1234567890',20000);
...
for i = 1:length(teclas) % For que percorre cada uma das teclas inseridas
    ...
    onda = sin((2*pi*DTMF.linhaTom(jlinha,jcoluna)*n)/fs) + cos((2*pi*DTMF.colunaTom(jlinha,jcoluna)*n)/fs); % DTMF Equation
...
soundsc(sinal, fs); % Função que reproduz o sinal gerado de acordo com fs

%% Time Domain

tplot = (0:1/fs:(length(sinal)-1)/fs); % Tempo para a plotagem do gráfico
figure(1)
plot(tplot,sinal);
grid on
set(gca,'FontSize',10)
...
Nfft = 2^nextpow2(length(sinal));
Fsinal = fft(sinal,Nfft);
...
audiowrite('seqDTMF.wav',sinal, fs); % Function that creates an audio file with the generated signal